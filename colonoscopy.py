...
uploaded_file = st.file_uploader(
    "请上传包含特定信息数据的表格文件:\nPlease upload a form file containing specific information data:",
    type=["csv", "xlsx"])
...
if uploaded_file is not None:
    ...
    def process_and_predict(data):
        ...
        for index, row in data.iterrows():
            # 填充输入数据
            input_dict = {
                'gender': row['性别(Gender)'], 'a1': row['年龄(Age)'], 'a2': row['婚姻状况(Marital status)'],
                'a3': row['受教育水平(Education)'], 'a4': row['职业(Occupation)'],
                'b1': row['慢性腹泻史(History of chronic diarrhea)'],
                'b2': row['慢性便秘史(History of chronic constipation)'],
                'b3': row['粘液和或血便史(History of mucus or blood in the stool)'],
                'b4': row['慢性阑尾炎或阑尾切除史(History of chronic appendicitis or appendectomy)'],
                'b5': row['慢性胆囊炎或胆囊切除史(History of chronic cholecystitis or cholecystectomy)'], 'b6': row[
                    '近10年来有无经历过对精神造成较大创伤或痛苦的事件(Psychological trauma or distress in the past 10 years)'],
                'b7': row['癌症史(History of cancer)'], 'b9': row['肠息肉史(History of colorectal polyps)'], 'b10': row[
                    '一级亲属(父、母、兄弟姐妹、子女)肠癌史(Family history of colorectal cancer in first-degree relatives)'],
                'b11': row['血吸虫病史(History of schistosomiasis)'],
                'riskassessment': row['危险度评估结果(Colorectal cancer risk assessment-based questionnaire)'],
                'c1': row['是否抽烟？(Smoking)'], 'FOBT': row['便隐血测试(FOBT screening)']
            }
            ...
            def coding_fun(input_df):
                ...
                input_df['a3'] = input_df['a3'].replace(
                    ['小学及以下(primary school and below)', '中专、中学(middle school)',
                     '大学、大专(university and above)'],
                    [1, 2, 3])
                input_df['a4'] = input_df['a4'].replace(
                    ['无业(Unemployed)', '企业(enterprises)', "事业单位(Government agency)", '农民(Farmer)',
                     '其他(Others)'], [1, 2, 3, 4, 5])
                ...
                input_df['riskassessment'] = input_df['riskassessment'].replace(['阴性(Negative)', '阳性(Positive)'],
                                                                                [0, 1])
                input_df['c1'] = input_df['c1'].replace(
                    ['过去吸(Past cigarette smoking)', '现在吸(Current cigarette smoking)',
                     '不吸(Never cigarette smoking)'], [0, 1, 2])
            ...
            def make_predict(input_df):
                ...
                with open(
                        "E:/上中医/研0/论文写作/结直肠癌行为预测/实验更新_20250105/网页工具/sklearn_GBM_best_model.sav",
                        "rb") as f:
                    model = pickle.load(f)
